...
const possibleEngineParts: Array<EnginePart> = [];
const symbols: Array<Symbol> = [];
let currentPossibleEnginePart: undefined | EnginePart;
...
input.forEach((row, i) => {
  for (let j = 0; j < row.length; j++) {
    if (/\d/.exec(row[j])) {
      ...
      if (currentPossibleEnginePart === undefined) {
        currentPossibleEnginePart = {
        ...
        currentPossibleEnginePart.number += currentNumber;
      }
    } else {
      if (currentPossibleEnginePart !== undefined) {
        possibleEngineParts.push(structuredClone(currentPossibleEnginePart));
        currentPossibleEnginePart = undefined;
      }
...
possibleEngineParts.forEach(part => {
  for (const symbol of symbols) {
    if (Math.abs(symbol.y - part.y) > 1) {
      continue;
    }

    for (let i = 0; i < part.number.length; i++) {
      if ((symbol.x - (part.x + i)) ** 2 + (symbol.y - part.y) ** 2 <= 2) {
        part.valid = true;
        symbol.closeEngineParts ??= [];
        symbol.closeEngineParts.push(part);
        return;
      }
    }
  }
...
const sumOfAllPartsNumbers = possibleEngineParts.reduce(
  (sum, part) => (part.valid ? sum + parseInt(part.number) : sum),
  0
);

console.log(`Sum of all parts numbers: ${sumOfAllPartsNumbers}`);

const sumOfAllGearRatios = symbols.reduce((sum, symbol) => {
  if (symbol.closeEngineParts !== undefined && symbol.closeEngineParts.length > 1) {
    return (
      sum +
      symbol.closeEngineParts.reduce((gearRatio, part) => gearRatio * parseInt(part.number), 1)
    );
  }
...
interface EnginePart extends Node {
  number: string;
  valid: boolean;
}

interface Symbol extends Node {
  symbolType: string;
  closeEngineParts?: Array<EnginePart>;
}
