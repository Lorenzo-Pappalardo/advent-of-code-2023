const firstExample = ['1abc2', 'pqr3stu8vwx', 'a1b2c3d4e5f', 'treb7uchet'];
...
const numberWords: ReadonlyArray<string> = [
  'one',
  'two',
  'three',
  'four',
...
function getSum(input: ReadonlyArray<string>, parseNumberWords: boolean = false): number {
  return input.reduce((res, line) => {
    ...
    const wholeNumber = parseInt(`${first}${last}`);
...
console.log(`Result of the first example: ${getSum(firstExample)} | Correct result: ${142}`);
console.log(`Result of the first input: ${getSum(input)} | Correct result: ${54644}`);
console.log(
  `Result of the second example: ${getSum(secondExample, true)} | Correct result: ${281}`
);
console.log(`Result of the second input: ${getSum(input, true)} | Correct result: ${53355}`);
