...
input.forEach(game => {
  const [gameIDDefinition, cubesExtractions] = game.split(':');
  const cubesExtractionArray = cubesExtractions.split(';');

  const gameIDMatches = /Game\s(\d+)/.exec(gameIDDefinition);

  if (gameIDMatches === null) {
    return;
  }

  const gameID = parseInt(gameIDMatches[1]);
  let valid = true;

  const minNumberOfCubesForEachColorNeededForEachGameToBePossible = {
    red: 0,
    green: 0,
    blue: 0
  };

  for (const extractedCubes of cubesExtractionArray) {
    const cubesOccurrences = /(?:\s(\d+\s\w+),?)?(?:\s(\d+\s\w+),?)?(?:\s(\d+\s\w+),?)/.exec(
      extractedCubes
    );

    if (cubesOccurrences === null) {
      return;
    }

    const maxNumberForEachColor = {
      red: 12,
      green: 13,
      blue: 14
    };
    ...
    for (let i = 1; i <= 3; i++) {
      const cubesOccurrence = cubesOccurrences[i];

      if (cubesOccurrence !== undefined) {
        const splits = cubesOccurrence.split(' ');
        const number = parseInt(splits[0]);
        const color = splits[1] as keyof typeof maxNumberForEachColor;

        if (number > minNumberOfCubesForEachColorNeededForEachGameToBePossible[color]) {
          minNumberOfCubesForEachColorNeededForEachGameToBePossible[color] = number;
        }

        if (number > maxNumberForEachColor[color]) {
          valid = false;
        }
      }
    }
  ...
  sumOfPowersOfGames += Object.values(
    minNumberOfCubesForEachColorNeededForEachGameToBePossible
  ).reduce((res, value) => res * value);
});

console.log(sumOfIDs);
console.log(sumOfPowersOfGames);
